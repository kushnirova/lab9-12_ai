...
<div class="container mx-auto px-4 max-w-2xl py-8">
    <h1 class="text-3xl font-bold mb-6 text-gray-800">Dodaj nową świnkę</h1>

    <div class="bg-white rounded-lg shadow p-6">
        <form action="{{ route('admin.guinea_pigs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="number" name="age" id="age" value="{{ old('age') }}" required min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">
            ...
            <div>
                ...
                <select name="status" id="status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2 bg-white">
                    <option value="available" {{ old('status') == 'available' ? 'selected' : '' }}>Dostępna</option>
                    <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>W trakcie adopcji</option>
                    <option value="adopted" {{ old('status') == 'adopted' ? 'selected' : '' }}>Zaadoptowana</option>
                </select>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="4" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 border p-2">{{ old('description') }}</textarea>
                @error('description') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="image_path" class="block text-sm font-medium text-gray-700">Zdjęcie</label>
            ...
            <div class="flex justify-end pt-4">
                ...
                <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Zapisz</button>
            </div>
        </form>
    </div>
</div>
